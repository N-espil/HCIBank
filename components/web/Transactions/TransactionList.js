        ...
        return (
            <div>
                <div className='flex w-full '>
                    {period == "Today" ?
                        <h1 className='w-full m-0 mb-4 text-3xl 4xl:mb-6 4xl:text-4xl text-medium text-primary xl:text-2xl'>Recent Transactions</h1>
                        :
                        <h1 className='w-full m-0 text-3xl font-bold 4xl:mb-6 4xl:text-4xl text-primary'>Transactions</h1>
                    }
                    {(session?.user.privilege == "MAIN" || session?.user.privilege == "MAIN2") &&
                        <div className='flex items-center gap-4 -mb-4'>
                            <HiUser onClick={() => setIsMyTransactions(true)}
                                className={`${!isMyTransactions ? "text-base-100 border-base-100" : "text-primary border-primary"} cursor-pointer w-8 h-8 m-0 text-3xl border rounded-full`}></HiUser>
                            <HiUserGroup onClick={() => setIsMyTransactions(false)}
                                className={`${isMyTransactions ? "text-base-100 border-base-100" : "text-primary border-primary"} cursor-pointer w-8 h-8 m-0 text-3xl border rounded-full`}></HiUserGroup>
                        </div>
                    }
                </div>
                <h1 className='text-2xl font-normal text-primary'>No transactions to show...</h1>
            </div>
        )
    }
    ...
    return (
        <>
            <div className='flex'>
                {period == "Today" ?
<>
                    <h1 className='w-full m-0 font-bold lg:text-2xl 2xl:text-3xl 4xl:text-4xl text-primary'>Recent Transactions</h1>
                        {(session?.user.privilege == "MAIN" || session?.user.privilege == "MAIN2") &&
                    ...
                    :
                    <>
                        <h1 className='w-full m-0 font-bold lg:text-2xl 2xl:text-3xl 4xl:text-4xl text-primary'>Transactions</h1>
                        {(session?.user.privilege == "MAIN" || session?.user.privilege == "MAIN2") &&
                            <div className='flex items-center gap-4 -mb-2 3xl:-mb-4'>
                                <HiUser onClick={() => setIsMyTransactions(true)}
                                    className={`${!isMyTransactions ? "text-base-100 border-base-100" : "text-primary border-primary"} cursor-pointer w-8 h-8 m-0 text-3xl border rounded-full`}></HiUser>
                                <HiUserGroup onClick={() => setIsMyTransactions(false)}
                                    className={`${isMyTransactions ? "text-base-100 border-base-100" : "text-primary border-primary"} cursor-pointer w-8 h-8 m-0 text-3xl border rounded-full`}></HiUserGroup>
                    ...
                    // <h1 className='w-full m-0 text-3xl 4xl:text-4xl text-bold text-primary xl:text-2xl'>Recent Transactions</h1>
                    ...
                    {transactions?.map((transactionGroup, index) => {
                        return (
                            <div key={index} className='w-full'>
                                ...
                                <h1 className='sticky top-0 z-30 w-full p-1 m-0 border-t border-base-100 lg:text-sm lg:px-2 2xl:text-base 2xl:px-4 4xl:text-xl 4xl:px-5 bg-neutral text-base-100'>{transactionGroup[0].date}</h1>
                                        ...
                                        <div key={index} className={`flex items-center justify-between lg:text-xs 2xl:text-base border-b border-neutral p-2 4xl:text-lg 4xl:px-5 text-primary`}>
                                            {(subIDs.includes(item.toId) && main2IDs.includes(item.fromId)) || (subIDs.includes(item.fromId) && main2IDs.includes(item.toId)) ?
                                                ...
                                                <div className='flex flex-col gap-4'>
                                                    <div className='flex items-center justify-start 4xl:gap-5 lg:gap-2 2xl:gap-3'>
